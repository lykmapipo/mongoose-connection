...
const connect = (url, done) => {
  ...
  if (isConnectedOrConnecting(conn)) {
    common.wrapCallback(cb)(null, conn);
  }
  // do: establish connection
  else {
    mongoose.connect(uri, options, common.wrapCallback(cb));
  }
};
...
const disconnect = (connection, done) => {
  ...
  // disconnect
  if (localConnection) {
    return localConnection.close(common.wrapCallback(cb, replyConnection));
  }
  return mongoose.disconnect(common.wrapCallback(cb, replyConnection));
};
